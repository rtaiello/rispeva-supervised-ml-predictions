...
def ensemble_bagging(X_train, y_train):
    classifier = svm.SVC(C=1.0, kernel='poly', degree = 2, random_state=SEED)
    clf = BaggingClassifier(base_estimator=classifier, n_estimators=300, n_jobs=-1)
    clf.fit(X_train, y_train)
    return clf

def ensemble_random_forest(X_train, y_train):
    clf = RandomForestClassifier(n_estimators=300, criterion='gini',max_features=10, n_jobs=-1,class_weight='balanced',
                               random_state=SEED)
...
def svm_classifier(X_train,y_train):
    ...
    clf = svm.SVC(C=1.0, kernel='poly', degree=3, class_weight='balanced', max_iter=-1,  random_state=SEED).fit(X_train, y_train)
    return clf
