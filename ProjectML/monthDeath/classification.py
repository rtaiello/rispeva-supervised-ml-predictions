...
def ensemble_bagging(X_train, y_train):
    classifier = AdaBoostClassifier(LogisticRegression(class_weight='balanced',n_jobs=-1), n_estimators=50, learning_rate=1.0, algorithm='SAMME')
...
def ensemble_ada_boosting(X_train, y_train):
    clf = AdaBoostClassifier(LogisticRegression(class_weight='balanced',n_jobs=-1), n_estimators=50, learning_rate=1.0, algorithm='SAMME')
    clf.fit(X_train, y_train)
    return clf
